...
class AgentHandler(StreamHandler):
    ...
    def emit(self) -> None:
        ...
        if self.initial_chunk:
            out_chunk_1 = out_chunk_2 = (np.zeros(self.chunk_size_samples, dtype=np.float32), None)
        ...
        out_chunk = np.vstack((out_chunk_1[0], out_chunk_2[0])).mean(axis=0, keepdims=True)
...
def main(args):
    ...
    agent_1 = RealtimeAgentMultiprocessing(
        ...
        self_play_mode=True,
        gpu_id=args.agent_1_gpu_id,
        wait_until_running=False,
    )
    agent_2 = RealtimeAgentMultiprocessing(
        llm_model_path=args.llm_model_path,
        self_play_mode=True,
        gpu_id=args.agent_2_gpu_id,
        wait_until_running=False,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None, 
        help="GPU ID to use for Agent 1 (None to use the first visible device).",
    )
    parser.add_argument(
        "--agent_2_gpu_id", 
        type=int,
        default=None, 
        help="GPU ID to use for Agent 2 (None to use the first visible device).",
    )
