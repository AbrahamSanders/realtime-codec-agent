...
from .utils import asr_handler_helpers
...
class ASRHandlerMultiprocessing:
    ...
    def _whisper_transcribe(self, model, config, next_input, cached_resample, last_n_segs=None):
        ...
        if prefix_audio:
            prefixed_next_input = asr_handler_helpers.concat_audios_to_tensor(prefix_audio + [next_input])
        else:
            prefixed_next_input = next_input

        audio = prefixed_next_input[0].to(device=model.device)
        audio, cached_resample = asr_handler_helpers.downsample(audio, prefixed_next_input[1], SAMPLE_RATE, cached_resample)
        ...
        while True:
            try:
                ...
                if len(input_buffer) >= buffer_size:
                    ...
                    next_input_last_fragment = asr_handler_helpers.concat_audios_to_tensor(input_buffer[-1:])
                    ...
                    next_input = asr_handler_helpers.concat_audios_to_tensor(input_buffer)
